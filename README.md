...
Phase 1 generates synthetic state-tracking datasets across six prompt shells:
...
```powershell
python scripts/01_generate_data.py --n 60 --seed 0 --output data/phase1/synthetic_state_tracking.jsonl
```
...
C:\Users\Kings\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe scripts/01_generate_data.py --n 60 --seed 0 --output data/phase1/synthetic_state_tracking.jsonl
New-Item -ItemType Directory -Force data/processed
Copy-Item data/phase1/synthetic_state_tracking.jsonl data/processed/dev.jsonl
...
```powershell
python scripts/01_generate_data.py --n 120 --seed 42 --z 1 --d 6 --k 3 --w 2 --output data/phase1/z1_d6_k3_w2.jsonl
```
...
```bash
python scripts/01_generate_data.py --n 1000 --seed 42 --z 1 --d 6 --k 3 --w 2 --output data/phase1/autodl_phase1.jsonl
```
...
- `z_accuracy`: macro exact-match accuracy grouped by `z`
- `d_accuracy`: macro exact-match accuracy grouped by `d`
- `k_accuracy`: macro exact-match accuracy grouped by `k`
- `w_preservation`: macro exact-match accuracy grouped by `w`
